...
(async()=>{
  try{
    ...
    let login = await request(baseOrigin + '/api/user/login','POST',{'Content-Type':'application/json'},JSON.stringify({email:TEST_EMAIL,password:TEST_PASS}));
    let token;
    if (login.status === 200) {
      token = login.body.token;
    } else {
      console.log('Registering user', TEST_EMAIL);
      const reg = await request(baseOrigin + '/api/user/register','POST',{'Content-Type':'application/json'},JSON.stringify({name:'Supe',email:TEST_EMAIL,password:TEST_PASS}));
    ...
    for (const u of afterUsers) {
      ...
      if (after !== before) changed.push({ email: u.email, before, after, delta: after - before });
    }
    ...
    if (!smoke) { console.error('Target user not changed'); process.exit(2); }
    if (others.length > 0) { console.error('Other users changed:', others); process.exit(3); }
