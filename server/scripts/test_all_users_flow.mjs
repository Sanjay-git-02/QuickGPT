...
(async()=>{
  try{
    ...
    const smokeChange = changed.find(c=>c.email==='smoketest@example.com');
    if (!smokeChange) { console.error('smoketest did not change'); process.exit(2); }
    ...
    if (smokeChange.delta !== 100) { console.error('Unexpected delta for smoketest', smokeChange); process.exit(4); }
