...
(async () => {
  try {
    ...
    if (!STRIPE_WEBHOOK_SECRET_KEY)
      throw new Error("Missing STRIPE_WEBHOOK_SECRET_KEY in server/.env");

    const base = "http://localhost:3000";

    console.log("Logging in smoketest user...");
    let r = await fetch(base + "/api/user/login", {
      method: "POST",
    ...
    r = await fetch(base + "/api/credits/purchase", {
      method: "POST",
      headers: {
    ...
    if (!sessionId)
      throw new Error("No sessionId returned by purchase endpoint");

    const stripe = new Stripe(STRIPE_SECRET_KEY);
    console.log("Retrieving session from Stripe:", sessionId);
    const session = await stripe.checkout.sessions.retrieve(sessionId);
    console.log(
      "Session retrieved. id=",
      session.id,
      "metadata=",
      session.metadata,
    );
    ...
    const wh = await fetch(base + "/api/stripe", {
      method: "POST",
      headers: {
    ...
    const ud = await fetch(base + "/api/user/data", {
